...
public class Board extends JLabel implements Observer {
	...
	private void drawSnake(Graphics g) {
		for (int i = 0; i != SnakeApp.MAX_THREADS; i++) {
			LinkedList<Cell> snakeBodyCopy;
			synchronized (SnakeApp.getApp().snakes[i]) {
				snakeBodyCopy = new LinkedList<>(SnakeApp.getApp().snakes[i].getBody());
			}
			for (Cell p : snakeBodyCopy) {
				if (p.equals(snakeBodyCopy.peekFirst())) {
